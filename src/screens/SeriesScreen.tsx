...
export function SeriesScreen() {
  ...
  const load = useCallback(async () => {
    ...
    const [cats, items, favIds, recIds] = await Promise.all([
      ...
      mediaService.getSeries(providerId, selectedCategoryId ?? undefined),
      mediaCollectionRepo.favoriteIds('series'),
      mediaCollectionRepo.recentIds('series', 50),
    ]);
    setCategories(cats);
    setAllItems(items);
  ...
  const baseItems = useMemo(() => {
    if (mode === 'favorites') return allItems.filter((s) => favoriteIds.has(s.id));
    if (mode === 'recent') {
      const map = new Map(allItems.map((i) => [i.id, i]));
      return recentIds.map((id) => map.get(id)).filter((v): v is SeriesItem => !!v);
    }
    return allItems;
  ...
  const openSeries = useCallback(async (id: string) => {
    ...
    const ids = await mediaCollectionRepo.recentIds('series', 50);
    setRecentIds(ids);
    // TODO: navigate to series detail / episodes when implemented
  }, []);
  ...
  return (
    <View className="flex-1" style={{ backgroundColor: '#0e0e12' }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginHorizontal: 16, marginVertical: 10, height: 40, paddingHorizontal: 12, borderRadius: 10, borderWidth: focusedKey === 'series-search' ? 3 : 1, borderColor: focusedKey === 'series-search' ? '#d8b4fe' : '#3f3f46', backgroundColor: '#1c1c24' }}>
